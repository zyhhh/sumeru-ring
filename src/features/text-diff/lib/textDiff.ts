...
export type VisibleDiffItem = { type: 'row'; row: DiffRow } | {
    type: 'gap';
    id: string;
    count: number;
    rows: DiffRow[]
...
function inlineDiff(left: string, right: string, ignoreWhitespace: boolean) {
    if (ignoreWhitespace && normalizeWhitespace(left) === normalizeWhitespace(right)) return {
        left: plain(left),
        right: plain(right)
    }
    const changes = diffWordsWithSpace(left, right)
    return {
        left: changes.filter(change => !change.added).map(change => ({
            text: change.value,
            kind: change.removed ? 'removed' : 'same'
        } as TextSegment)),
        right: changes.filter(change => !change.removed).map(change => ({
            text: change.value,
...
export function compareText(original: string, modified: string, ignoreWhitespace = false): TextDiffResult {
    ...
    for (let index = 0; index < changes.length;) {
        ...
        if (!change.added && !change.removed) {
            for (const text of change.value) rows.push({
                id: rowId++,
                kind: 'same',
                left: {lineNumber: leftLine++, segments: plain(text)},
                right: {lineNumber: rightLine++, segments: plain(text)}
            })
            index += 1;
            continue
        }
        ...
        differenceId += 1;
        differenceIds.push(differenceId)
        ...
        for (let offset = 0; offset < Math.max(removedLines.length, addedLines.length); offset += 1) {
            const left = removedLines[offset];
            const right = addedLines[offset]
            if (left !== undefined && right !== undefined) {
                ...
                rows.push({
                    ...
                    kind: 'modified',
                    differenceId,
                    left: {lineNumber: leftLine++, segments: segments.left},
                    right: {lineNumber: rightLine++, segments: segments.right}
                })
            } else if (left !== undefined) rows.push({
                id: rowId++,
                kind: 'removed',
                differenceId,
                left: {lineNumber: leftLine++, segments: [{text: left, kind: 'removed'}]}
            })
            else if (right !== undefined) rows.push({
                id: rowId++,
                kind: 'added',
                differenceId,
                right: {lineNumber: rightLine++, segments: [{text: right, kind: 'added'}]}
            })
        }
